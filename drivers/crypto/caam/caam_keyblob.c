...
static long kb_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CAAM_KB_GET_MONOTONIC_COUNTER:
	    {
		if (kb_data.keymod_len >= sizeof(u64)) {
			u64 snvs_lpsmc;
			void __user* userbuf = kb_data.keymod;
			retval = snvs_secvio_get_monotonic_counter(&snvs_lpsmc);
			if (retval == 0) {
				copy_to_user(userbuf,&snvs_lpsmc,sizeof(u64));
			}
		}
		break;
	    }
	case CAAM_KB_INC_MONOTONIC_COUNTER:
	    {
		retval = snvs_secvio_inc_monotonic_counter();
		break;
	    }
